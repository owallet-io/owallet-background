...
const handleGetDefaultAddressMsg: (service: KeyRingService) => InternalHandler<GetDefaultAddressTronMsg> = (service) => {
  return async (_, msg) => {
    ...
    let base58 = Address.getBase58Address(getAddressFromBech32(new Bech32Address(key.address).toBech32((await service.chainsService.getChainInfo(msg.chainId)).bech32Config.bech32PrefixAccAddr)));
